      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={motionVariants.container} className="text-center mb-16">
          ...
          <motion.p variants={motionVariants.item} className="responsive-text-xl text-muted-foreground max-w-3xl mx-auto">
            Insights, tutorials, and thoughts on web development, design trends, and the latest technologies shaping the digital landscape.
          </motion.p>
        </motion.div>

        {/* Tag Filter */}
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={motionVariants.fadeUp} className="flex flex-wrap justify-center gap-3 mb-12">
          <button onClick={() => setSelectedTag(null)} className={cn("px-4 py-2 rounded-full transition-all duration-300 text-sm font-medium", selectedTag === null ? "bg-primary text-primary-foreground shadow-lg" : "glass-card hover:bg-white/20 text-muted-foreground hover:text-foreground")}>
            All Posts
          </button>
          {allTags.map((tag) => (
            <button key={tag} onClick={() => setSelectedTag(tag)} className={cn("px-4 py-2 rounded-full transition-all duration-300 text-sm font-medium", selectedTag === tag ? "bg-primary text-primary-foreground shadow-lg" : "glass-card hover:bg-white/20 text-muted-foreground hover:text-foreground")}>
              {tag}
            </button>
          ))}
        </motion.div>
        ...
        {displayedPosts.length > 0 && (
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={motionVariants.fadeUp} className="mb-16">
            ...
            <GlassCard className="overflow-hidden group cursor-pointer hover:bg-white/15 transition-all duration-300">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                ...
                <div className="p-8 flex flex-col justify-center">
                  ...
                  <h3 className="text-2xl font-bold mb-4 group-hover:gradient-text transition-all duration-300">{displayedPosts[0].title}</h3>
                  <p className="text-muted-foreground mb-6">{displayedPosts[0].excerpt}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayedPosts.slice(1).map((post, index) => (
            <motion.article key={post.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: true }} whileHover={{ y: -5 }} className="group cursor-pointer">
              <GlassCard className="overflow-hidden h-full">
                ...
                <div className="p-6">
                  ...
                  <h3 className="font-bold mb-3 group-hover:gradient-text transition-all duration-300 line-clamp-2">{post.title}</h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {post.tags.length > 2 && <span className="px-2 py-1 bg-muted rounded-full text-xs font-medium">+{post.tags.length - 2}</span>}
                  </div>
        ...
        {hasMorePosts && (
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mt-12">
            <Button variant="outline" size="lg" onClick={loadMorePosts} className="group">
              Load More Articles
              <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
            </Button>
          </motion.div>
        ...
        {/* Blog Stats */}
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={motionVariants.fadeUp} className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
